...
abstract class RoutingServiceProvider extends ServiceProvider
{
    ...
    public function map(Router $router)
    {
        ...
        $router->group(['namespace' => $this->namespace, 'prefix' => LaravelLocalization::setLocale() ], function (Router $router) {
    ...
    private function loadFrontendRoutes(Router $router)
    {
        ...
        if ($frontend && file_exists($frontend)) {
            $router->group(['middleware' => config('core.core.middleware.frontend', [])], function (Router $router) use ($frontend) {
                require $frontend;
            });
        }
    }
    ...
    private function loadBackendRoutes(Router $router)
    {
        ...
        if ($backend && file_exists($backend)) {
            $router->group(['namespace' => 'Admin', 'prefix' => config('core.core.admin-prefix'), 'middleware' => 'auth'], function (Router $router) use ($backend) {
                require $backend;
            });
        }
    }
    ...
    private function loadApiRoutes(Router $router)
    {
        ...
        if ($api && file_exists($api)) {
            $router->group(['namespace' => 'Api', 'prefix' => 'api', 'middleware' => config('core.core.middleware.api', [])], function (Router $router) use ($api) {
                require $api;
            });
        }
    }
